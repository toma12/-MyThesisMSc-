...
\section{Wake Turbulence Separation Minima}
...
Traditional separation standards, introduced in the seventies, are defined by ICAO based on certificated maximum take-off mass (MTOM) of aircraft. The MTOM criteria allocates each aircraft in a wake turbulence category. The prescribed categories are three i.e. Heavy, Medium and Light.
...
The resulting categories following the criteria for re-categorisation are six:\\
\begin{tabular}{l}
...
The RECAT-EU wake separation matrix combining leader and follower into pairs with assigned wake separation minima for each pair is shown in Table~\ref{tab:RECAT-dist}.
