...
\subsection{Runway Occupancy Time\label{sec:ROT}}
...
\fxnote{equipment specifications in the tower that might influence the WTC separtarion decision-talk to Haraldur}

\subsection{Runway Occupancy and Landing Time Interval Study Objective}
The objective of examining the arrival runway occupancy times and the landing time intervals (LTI) is to determine whether the implementation of the RECAT-EU scheme at BIKF under certain conditions, would affect the throughput of the airfield. Additionally the analysis could indicate whether limiting element is the runway occupancy or the separation requirements. The RECAT scheme suggests a decrease in distance and in time separation of aircraft pairs (\ref{tab:delta_distance_wtc2recat}, \ref{tab:delta_times_wtc2recat})
