...
It must be noted that the division of ICAO WTC categories into the RECAT scheme is not achieved by splitting each category in half.
The criteria used for categorisation of existing and new aircraft types are provided in detail in~\cite{noauthor_recat_2018}. The case from the traffic mix at BIKF presents the arrangement shown in Table~\ref{tab:wtc2recat_division}. A more detailed illustration of the re-categorisation is presented in the coming section \ref{ssec:traffic_mix} on aircraft traffic mix (Figure~\ref{fig:post_fast_exit_mix_pie_v2}).
...
\fxnote{explain the transition from WTC to REACT , and point the case for BIKF: H to ABC, M to CDEF, ....reference the pie chart on mix}
...
The implementation of RECAT shifts the wake separation minima for arrival pairs thus creating the potential for reduced time separation between the aircraft. The reduction of the time interval between landings facilitates increas in the throughput capacity of the runways. 
