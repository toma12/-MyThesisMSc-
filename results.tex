...
Despite the reduced AROT, runway RWY~28 remains least used, servicing only 10$\%$ of landing aircraft (Figure~\ref{fig:runway_usage_peak}). One explanation for the small share of landings on RWY-28 is that flights on approach for landing from the east are generally uncommon for BIKF. 
...
A statistical summary for all the runways is shown in Table~\ref{tab:all_RWY_AROT_stats}. The average AROT for the BIKF airfield amounted to 77,5 seconds with average standard deviation about the mean of 14.6 seconds. Overall, half of the landing aircraft occupy the runways between 67 and 85 seconds on average. 
